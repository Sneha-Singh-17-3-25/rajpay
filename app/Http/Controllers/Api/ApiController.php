...
class ApiController extends Controller
{
    ...
    {
        $user = User::where('id',$post->user_id)->first(['mainwallet']);
        if(!$user){
            return response()->json(['statuscode' => 'ERR', 'message' => 'Please contact service provider']);
        }else{
            return response()->json(['statuscode' => "TXN", "message" => "Balance Fetched Successfully", "balance" => $user->mainwallet]);
        }
    }
    ...
    {
        return response()->json(['statuscode' => "TXN", "message" => "Balance Fetched Successfully", 'ip'=> $post->ip()]);
    }
}