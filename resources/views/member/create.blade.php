                    ...
                    <div class="panel-body p-b-0">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="gstin" class="form-control" value="" placeholder="Enter Value">
                            </div>

                            @if(Myhelper::hasRole('admin') && sizeof($scheme) > 0)
                                <div class="form-group col-md-4">
                                    <label>Scheme</label>
                    ...
                    <div class="panel-body p-b-0">
                        ...
                        <div class="row">
                            <div class="form-group col-md-12">
                                ...
                                <textarea name="shopaddress" class="form-control" rows="2" required="" placeholder="Enter Value"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('[name="role_id"]').on('change', function(){
            var role = $(this).val();
            if(role != ''){
                var inputdata = {"roleid" : role};
                SYSTEM.AJAX('{{route("getparent")}}', "POST", inputdata, function(data){
                    if(!data.statusText){
                        if(data.status == "success"){
                            if(data.data.length > 0){
                                var out = '<option value="">Select Parent</option>';
                                $.each(data.data, function(index, values) {
                                    out +=`<option value="`+values.id+`">`+values.name+` (`+values.id+`) (`+values.role.slug+`)</option>`;
                                });
                            }
                            $('[name="parent_id"]').html(out);
                        }else{
                            SYSTEM.SHOWERROR(data, $('.memberForm'), 'popup');
                        }
                    }else{
                        SYSTEM.SHOWERROR(data, $('.memberForm'), 'popup');
                    }
