                ...
                <table class="table table-bordered table-striped table-hover" id="datatable">
                    <thead>
                        <tr>
                            ...
                            @if(in_array($type, ["whitelable", "md", "distributor"]))
                                <th>Stock</th>
                            @endif
                            <th>Action</th>
                        </tr>
...
@if(!in_array($type, ["kycpending", "kycsubmitted", "kycrejected"]))
                ...
                <form id="transferForm" action="{{route('fundtransaction')}}" method="post">
                    <div class="modal-body">
                            ...
                            @if (Myhelper::hasRole('admin'))
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="wallet" class="form-control select" id="select" required>
                                        ...
                                        <option value="aepswallet">Aeps Wallet</option>
                                    </select>
                                </div>
                            @else
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                <label>Refno</label>
                                <input type="text" name="refno" class="form-control" placeholder="Enter value" required="">
                            </div>
                        </div>
                        <div class="row">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var onDraw = function() {
            $('input#membarStatus').on('click', function(evt){
                ...
                $.ajax({
                    ...
                    dataType:'json',
                    data: {'id':id, 'status':status, 'actiontype' : 'memberstatus'}
                })
                .done(function(data) {
        ...
        var options = [
            ...
            { "data" : "name",
                render:function(data, type, full, meta){
                    return `<span class="name">`+full.name+`</span>` +`<br>`+full.mobile+`<br>`+full.agentcode;
                }
            },
            { "data" : "parents"},
            { "data" : "name",
                render:function(data, type, full, meta){
                    return full.company.companyname+`<br>`+full.company.website;
                }
            },
            { "data" : "name",
                render:function(data, type, full, meta){
                    return `Main : `+full.mainwallet + `<br>Aeps : `+ full.aepswallet + `<br>Locked : `+ full.lockedamount;
                }
            },
            @if(in_array($type, ["whitelable", "md", "distributor"]))
            { "data" : "name",
                render:function(data, type, full, meta){
                    @if ($type == "whitelable" || $type == "other")
            ...
            { "data" : "action",
                render:function(data, type, full, meta){
                    ...
                    @if(!in_array($type, ["kycpending", "kycsubmitted", "kycrejected"]))
                        ...
                        @if (Myhelper::can("locked_amount"))
                            menu += `<li><a href="javascript:void(0)" onclick="lockedAmount('`+full.id+`', '`+full.lockedamount+`')"><i class="fa fa-inr"></i>Locked Amount</a></li>`;
                        @endif
                        ...
                        @if (Myhelper::can('service_manager'))
                            menu += `<li><a href="{{url('setup/servicemanage')}}/`+full.id+`" target="_blank"><i class="icon-user"></i>Service Manager</a></li>`;
                        @endif
                    ...
                    @if(!in_array($type, ["kycpending", "kycsubmitted", "kycrejected"]))
                        @if (Myhelper::can(['member_aeps_statement']))
                            menu2 += `<li><a href="{{url('statement/report/aeps/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Aeps</a></li>`;
                            menu2 += `<li><a href="{{url('statement/report/aadharpay/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Aadhar Pay</a></li>`;
                            menu2 += `<li><a href="{{url('statement/report/ministatement/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Mini Statement</a></li>`;
                            menu2 += `<li><a href="{{url('statement/report/matm/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Matm Statement</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_billpay_statement']))
                            menu2 += `<li><a href="{{url('statement/report/billpay/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Billpayment</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_recharge_statement']))
                            menu2 += `<li><a href="{{url('statement/report/recharge/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Recharge</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_dmt_statement']))
                            menu2 += `<li><a href="{{url('statement/report/dmt/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Money Transfer</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_utipancard_statement']))
                            menu2 += `<li><a href="{{url('statement/report/pancard/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Utipancard</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_utilityLedger_statement']))
                            menu2 += `<li><a href="{{url('statement/report/ledger/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Account Statement</a></li>`;
                        @endif

                        @if (Myhelper::can(['member_aepsLedger_statement']))
                            menu2 += `<li><a href="{{url('statement/report/aepsledger/')}}/`+full.id+`" target="_blank"><i class="icon-paragraph-justify3"></i> Aeps Wallet Statement</a></li>`;
                        @endif
                    ...
                    @if (Myhelper::can('member_profile_edit'))
                        out3 +=  `<ul class="icons-list">
                            <li class="dropdown">
                                <a href="{{url('profile/view')}}/`+full.id+`" class="mt-10" target="_blank">
                                    <span class="label bg-slate">View Profile</span>
                                </a>
                            </li>
                        </ul>`;
                    @endif
