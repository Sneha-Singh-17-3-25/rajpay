...
@php
    $table    = "yes";
    $search   = "hide";
    $category = ["electricity", "water", "lpggas", "gasutility", "loanrepay", "lifeinsurance", "insurance", "postpaid", "muncipal", "tax", "housing"];
    asort($category);
@endphp
    ...
    <div class="tabbable">
        <ul class="nav nav-tabs bg-slate nav-tabs-component">
            ...
            @foreach ($category as $category)
                <li><a href="#recharge" data-toggle="tab" id="{{$category}}Tab" class="legitRipple" aria-expanded="false" onclick="SETTITLE('{{$category}}', 'billpayForm', 'fetchpay')">{{ucfirst($category)}}</a></li>
            @endforeach
        </ul>
...
@push('script')
    ...
    <script type="text/javascript">
        ...
        function SETTITLE(type, form, billPayType) {
            ...
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data : {"type" : type, "billPayType" : billPayType},
                headers: {
        ...
        function GET_PROVIDER(form) {
            ...
            if(providerid != ''){
                ...
                .done(function(data) {
                    swal.close();
                    $("#"+form).find('.billdata').empty();
                    var paramcount = data.data.paramname.length;
            ...
            if($("#"+form).find('[name="provider_id"]').val() != ''){
                ...
                .done(function(data) {
                    ...
                    if(data.statuscode == "TXN"){
                        ...
                        $('#billpayForm').find('[name="amount"]').val(data.data.dueamount);
                        $('#billpayForm').find('.billdata').append(`
                            <div class="form-group col-md-4">
                                <label>Due Amount</label>
                                <input type="text" class="form-control amount" readonly="" placeholder="Enter amount" required="" value="`+data.data.dueamount+`">
                                <b>Name - </b>`+data.data.customername+`, Duedate - `+data.data.duedate+`
                            </div>`);
