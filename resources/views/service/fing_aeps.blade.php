@extends('layouts.app')
@section('title', "Aeps (Bank-1) Service")
@section('pagetitle', "Aeps (Bank-1) Service")
                ...
                <div class="col-md-10 col-md-offset-1">
                    <div class="tabbable">
                        <ul class="nav nav-tabs bg-slate-600 nav-tabs-component">
                            <li class="active">
                                <a href="#recharge" data-toggle="tab" id="BETab" class="legitRipple" aria-expanded="false" onclick="AEPSTAB('BE')">Balance Enquiry
                                </a>
                            </li>

                            <li>
                                <a href="#recharge" data-toggle="tab" id="MSTab" class="legitRipple" aria-expanded="false" onclick="AEPSTAB('MS')">Mini Statement
                                </a>
                            </li>

                            <li>
                                <a href="#recharge" data-toggle="tab" id="CWTab" class="legitRipple" aria-expanded="false" onclick="AEPSTAB('CW')">Cash Withdrawal
                                </a>
                            </li>

                            <li>
                                <a href="#recharge" data-toggle="tab" id="MTab" class="legitRipple" aria-expanded="false" onclick="AEPSTAB('M')">Aadhar Pay
                                </a>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="recharge">
                                <div class="panel panel-body auth_cw_panel p-10" style="display: none;">
                                ...
                                <div class="panel panel-default">
                                    <form action="{{route('aepspay')}}" method="POST" id="aepsTransactionForm" enctype="multipart/form-data">
                                        ...
                                        <input type="hidden" name="auth" value="{{$agent->auth}}">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="pull-right m-t-10">
                                        <address class="m-b-10">
                                            <strong>Bank : </strong> <span class="biller"></span><br>
    ...
    <div id="authenticationModal" class="modal fade" data-backdrop="false" data-keyboard="false">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    <div class="panel panel-default">
                        <form action="{{route('aepskyc')}}" method="POST" id="aeps2FAForm" enctype="multipart/form-data">
                            {{ csrf_field() }}

                            <input type="hidden" name="transactionType" id="transactionType" value="authentication">
                            <input type="hidden" name="serviceType" value="AEPS">
                            <input type="hidden" name="biodata" value="">
                            <input type="hidden" name="gps_location">
...
<script src="{{ asset('assets/js/core/jquery-confirm.min.js') }}"></script>
<script src="{{ asset('assets/js/core/notify.min.js') }}"></script>
<script src="{{ asset('assets/js/core/jquery.cookie.js') }}"></script>
<script src="{{ asset('assets/js/core/aadhaar_capture.js') }}"></script>


    <script type="text/javascript">
        var gif = '{{ asset("assets/images/capute.gif") }}',
    loading = '{{ asset("assets/images/loading.gif") }}';
        ...
        $(document).ready(function () {
            ...
            $( "#aepsTransactionForm" ).validate({
                ...
                submitHandler: function (element) {
                    ...
                    if(auth == "need" || (transactionType == "CW" && auth_cw == "no")){
                        $("#authenticationModal").modal();
                    }else{
                        if(scan != ''){
                            if(lat != ''){
                                    ...
                                    if(!data.statusText){
                                        ...
                                        if(data.statuscode == "TXN" || data.statuscode == "TUP" || data.statuscode == "TXF"){
                                            ...
                                            if(data.transactionType != "MS"){                                   
                                                if(data.transactionType == "CW" || data.transactionType == "M" || data.transactionType == "BE"){
                                                    $(".cash").show();
                                                }else{
                                                    $('.cash').hide();
                                                }
                                                ...
                                                $('#ministatement').find('.biller').text(data.biller);
                                                $('#ministatement').find('.balance').text(data.balance);
                                                $('#ministatement').find('.title').text(data.provider);
                                                $('#ministatement').find('.message').text(data.message);
                                                var trdata = '';
        ...
        function AEPSTAB(type){
            if(type == "CW" || type == "M"){
                $('.transactionAmount').html(`
                    <label>Amount :</label>
                    <input type="text" class="form-control" name="transactionAmount" onkeyup="checkAmt()" pattern="[0-9]*" id="amount" autocomplete="off" placeholder="Enter Amount">
        ...
        function checkAmt(){
            var type   = $("#aepsTransactionForm" ).find('[name="transactionType"]').val();
            var amount = $("#aepsTransactionForm" ).find("[name='transactionAmount']").val();
            var remainder = (amount % 50);

            if(type == "CW"){
                $("#aepsTransactionForm").find('p.error').remove();
                if(remainder != 0){
                    $("#aepsTransactionForm").find('[name="transactionAmount"]').closest('div.form-group').append('<p class="error">Amount Should Be In Multiples Of 50</span>');
                    setTimeout(function () {
                        $("#aepsTransactionForm").find('p.error').remove();
                    }, 5000);
                }
