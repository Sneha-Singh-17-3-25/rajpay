@extends('layouts.app')
@section('title', "NPS Statement")
@section('pagetitle',  "NPS Statement")
            ...
            <div class="col-sm-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h4 class="panel-title">NPS Statement</h4>
                    </div>
...
@push('script')
    <script type="text/javascript">
        $(document).ready(function () {
            var url = "{{url('statement/list/fetch')}}/{{$type}}statement/{{$id}}";
            var onDraw = function() {
            };
            var options = [
                { "data" : "name",
                ...
                { "data" : "bank",
                    render:function(data, type, full, meta){
                        return `Name - `+full.name+`<br>Email - `+full.email+`<br>Mobile - `+full.mobile+`<br>District - `+full.district;
                    }
                },
                { "data" : "status",
                    render:function(data, type, full, meta){
        ...
        function viewUtiid(id){
            $.ajax({
                url: `{{url('statement/list/fetch')}}/utiidstatement/`+id+`/view`,
                type: 'post',
                headers: {
        ...
        function mydelete(id){
            $.ajax({
                url: `{{url('statement/delete')}}/utiidstatement/`+id,
                type: 'post',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
